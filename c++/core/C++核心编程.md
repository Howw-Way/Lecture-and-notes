...
{
	SheepTuo st;
	//m_Age均是来自于父类动物，故sheep和tuo两个中间父类均有该变量，此时通过指定具体的作用域以避免二义性，但注意，指定后是羊类和驼类的年龄，不是羊驼类的年龄，故无法使用st.m_Age
	//但即使避免了二义性，羊驼应该只有一个年龄，故其实只需要继承一个m_Age，而非两个
	st.Sheep::m_Age = 100;
	st.Tuo::m_Age = 200;

	cout << "st.Sheep::m_Age = " << st.Sheep::m_Age << endl;//未使用虚继承100，使用虚继承200（因为只有一份m_Age了，第二次赋值的时候将该变量的值改变了）
	cout << "st.Tuo::m_Age = " <<  st.Tuo::m_Age << endl;//未使用虚继承200，使用虚继承200
	//当使用虚继承后，只剩下一封m_Age
...
! [6](figure/6.jpg)
本质：
vbptr--虚基类指针(v-virtual,b-base,ptr-pointer)，指向虚基类表(vbtable)
例如Sheep中的虚基类指针，指向了$vbtable@Sheep@，该表格记录了一个偏移量(8)，由于其起始位置是0，+8后即可到达virtual base animal
同理，对于Tuo中的虚基类指针，指向了$vbtable@Tuo@，该表格记录了一个偏移量(4)，由于其起始位置是4，+8后亦可到达virtual base animal
...
地址早绑定：在该案例中，父类中的speak函数如果没加virtual，则将在编译阶段确定函数的地址，则在Dospeak函数中，animal.speak()，相当于调用的父类中的函数，即使子类中有同名函数
地址晚绑定：如果父类中的speak函数加了virtual，则将不会在编译阶段确定函数的地址（动态编译），则会调用传入对象的函数
...
class Cat :public Animal//猫继承于animal
{
public:
	void speak()
	{
...
//执行说话的函数 
void DoSpeak(Animal & animal)//当使用DoSpeak(cat)，相当于Animal & animal=cat，c++允许父类和子类中间进行类型转换，所以父类引用此时指向子类
{
	animal.speak();//如果父类中的speak加了virtual，则此时该处的调用，呈现出多种可能状态，由具体调用的类决定
}
...
总结：
动态多态可以理解成，父类子类中有相同的函数（且父类中的该函数是虚函数，子类是否是虚函数不影响），在编译阶段该函数有多种状态，最终确定状态是看调用该函数的对象具体是哪个，则使用该对象中的函数
...
* 父类指针或引用指向子类对象
(例如：Animal & animal=cat, Animal * animal=new Cat 注意new后接的是Cat类，而不是类的具体对象。具体例子也可以参考4.7.3案例的多态实现中，加法和减法的区别)

重写：函数返回值类型  函数名 参数列表 完全一致称为重写

#### 4.7.2 多态的原理

当不使用虚函数时，父类中的函数所占字节：1，相当于一个空函数（4.3.1 知识复习：成员函数不在类的空间里）
当使用虚函数后，父类中虚函数所占字节为：4，存放的是一个指针

![](./figure/8.jpg)
![](./figure/10.jpg)
图解释：先看父类Animal，其使用了virtual之后，有了一个vfptr(虚函数指针)指向了vftable(虚函数表，记录了虚函数的地址:&Animal::speak)
当子类没有发生函数重写的时候，将直接继承父类的虚函数，同样也有相同的vfptr和vftable

![](./figure/9.jpg)
![](./figure/11.jpg)
!{}](./figure/12.jpg)

而当子类重写了父类的虚函数，子类中的虚函数表内部会替换成子类的虚函数地址
当父类的指针或者引用指向子类对象时候，发生多态
即父类指针或引用指向子类对象后，此时调用的是子类中的成员函数
...
public:
	int getResult(string oper)
	{
		...
		//如果要提供新的运算，需要修改源码（此处修改源码指的是修改Calculator内部代码）
		//而在真是开发中，提倡开闭原则：对拓展进行开发，对修改进行关闭
	}
public:
	int m_Num1;
	int m_Num2;
...
public:
	//纯虚函数
	//类中只要有一个纯虚函数就称为抽象类
	//抽象类无法实例化对象 例如Base b; 将报错
	//子类必须重写父类中的纯虚函数，否则也属于抽象类，则也无法实例化对象
	virtual void func() = 0;
};
...
多态使用时，父类指针会指向子类，如果子类中有属性开辟到堆区，那么父类指针在释放时无法调用到子类的析构代码，即父类指针释放时只释放了父类所占内存，而不能释放子类在堆区开辟的内存，如果子类中在堆区开辟内存了，但又无法释放，将会造成了内存泄漏

问题产生原因：使用多态，是父类指针指向了子类，在delete父类时，并不会调用子类析构代码
解决方法：将父类的析构函数写成虚函数，则相当于使用虚函数解决了调用函数问题(本质与虚函数相同，虚函数解决了父类中同名函数早绑定导致不会调用子类同名函数的问题)

虚析构：对析构函数加上virtual
纯虚析构：虚析构函数=0；
注意：析构函数是要实际执行的，如果仅改成虚析构，则其在编译时，先调用子类中的析构函数，再会调用父类的析构函数，完成对堆区的释放。但是如果在父类中使用了纯虚析构，纯虚析构不会被执行，则会出问题，此时需要在类外进行调用（可以理解为父类中的纯虚函数需要在子类中重写，否则子类也是抽象类），调用方法：MakeDrink:: ~MakeDrink(){};(注意，要在父类外进行调用)
...
{
public:
	Tea(string name,int age) {
		cout << "Tea 构造函数" << endl;
		CoffeeName = new string(name);
		Age = new int (age);
...
void test01(){
	MakeDrink* make1 = new Tea("Lidun",10);
	make1->Drink ();
	delete make1;
...
//如果对父类析构函数使用虚函数：即virtual ~MakeDrink()
//此时输出为：
//父类构造函数调用
//Tea 构造函数
//Boil water for tea
...
* 如果是纯虚析构，该类属于抽象类，无法实例化对象(即使没有纯虚函数，也属于抽象类)
...
public:
	void Storage() {
		cout << "IntelStorage running" << endl;
	}
};

void test01() {
	//Computer IntelComp1(IntelCpu *c1, IntelVideoCard *vc1, IntelMemory *me1);
	CPU* c1= new (IntelCpu);
	VideoCard* vc1=new (IntelVideoCard);
	Memory* me1 = new(IntelMemory);
	Computer IntelComp1(c1, vc1, me1);
	IntelComp1.workflow();
	//delete me1;
	//delete c1;
	//delete vc1;
...
void test01()
{
	...
	//第四种，不推荐，每次读一个字符
	// char c;
	// while ((c = ifs.get()) != EOF)//EOF：end of file, 即文件尾
	// {
	// 	cout << c;
	// }
...
void test01()
{
	...
	//4、写文件
	ofs.write((const char *)&p, sizeof(p));//注意此处的(const char *)&p，p是指向person类的，而write要求的是一个指向char类的指针，所以使用了强制类型转换
...
选中该对象，右键“转到定义”，如果是在当前代码下的对象，则会跳转到该位置，如果在头文件中，则查找符号结果窗口出现头文件的位置
